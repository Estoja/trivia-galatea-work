...
- [ ] T096 [US1] [FR-033] Implementar control de sesgo de `correctOptionIndex` para preguntas generadas por IA (tema elegido y fallback Galatea) en `src/app/infrastructure/question/question.service.ts` y/o `src/app/infrastructure/helpers/maps/gemini-question.mapper.ts`: en cada lote de 6 preguntas IA no permitir concentración dominante en un único índice (`>3` en la misma posición 0..3); si ocurre, reordenar opciones preservando la respuesta correcta o regenerar lote antes de iniciar partida.
...
- [ ] T098 [P] [FR-033] Añadir regresiones unitarias para distribución de índices correctos en preguntas IA (sin afectar banco manual) en `src/app/infrastructure/question/question.service.spec.ts` y/o `src/app/infrastructure/helpers/maps/gemini-question.mapper.spec.ts`.
- [ ] T099 [P] [FR-034] Añadir regresiones unitarias para moderación de temas sensibles (bloqueos explícitos + casos neutros permitidos + falsos positivos) en `src/app/infrastructure/gemini/topic-safety-policy.spec.ts` (crear si no existe).

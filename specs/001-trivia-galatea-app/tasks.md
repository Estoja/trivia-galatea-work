...
- **`- [~]`**: protocolo/mecanismo de la tarea ya está documentado y listo, pero la **ejecución en vivo** (requiere Gemini real, credenciales, o el cierre real del evento) sigue **pendiente** — no confundir con `[X]` (completada) ni `[ ]` (sin iniciar).
...
- [X] T049 [US2] [FR-022] Implementar página `board` (tablero de 12 `tg-question-card`, apertura de `tg-question-modal`, navegación automática a `results` tras 6ª respuesta FR-012, máximo una pregunta activa a la vez FR-022) en `src/app/ui/pages/board/board.page.ts` (depende de T046, T047, T048)
...
- [X] T075 Ejecutar [quickstart.md](./quickstart.md) completo (modo mock y modo real con Gemini) y corregir discrepancias — comandos verificados (`npm test`, `npm run test:a11y`, `npm run e2e`, `npm run lint`, ambos builds); sin discrepancias encontradas
- [X] T076 [P] [DOC] Revisar y actualizar `README.md` del proyecto con instrucciones de instalación y ejecución (tarea documental/operativa; no mapea FR/SC funcional) — creado `README.md` (no existía)
- [~] T077 [P] Definir y ejecutar protocolo de validación manual de pertinencia para preguntas generadas por IA (muestra representativa de partidas, rúbrica de relevancia por tema, y criterio de aprobación) para demostrar SC-002 (95%) en `specs/001-trivia-galatea-app/checklists/ai-topic-questions.md` y evidencia en `specs/001-trivia-galatea-app/research.md` — **protocolo definido y documentado** en [research.md §9.4](./research.md), pero **NO ejecutado todavía**: requiere invocar Gemini real (no disponible en este entorno de desarrollo). ACCIÓN MANUAL PENDIENTE antes del evento: responsable del evento debe correr ≥10 temas libres variados contra el modo producción real y completar la rúbrica de `checklists/ai-topic-questions.md`
- [~] T078 [P] Instrumentar y medir latencia de generación de preguntas IA (percentil p90) en entorno de pruebas controlado, documentando resultados y umbral de aceptación para demostrar SC-003 (<=8s en 90% de partidas) en `src/app/infrastructure/gemini/gemini-client.service.ts` y evidencia en `specs/001-trivia-galatea-app/research.md` — **protocolo definido y documentado** en [research.md §9.4](./research.md), pero **NO ejecutado todavía**: requiere invocar Gemini real. ACCIÓN MANUAL PENDIENTE antes del evento: responsable del evento debe registrar ≥20 llamadas reales a `GeminiClientService.generateJson`, calcular p90 y confirmar que está dentro del umbral aceptable
- [X] T085 [P] Test e2e + unit de reinicio de partida ante recarga/cierre (FR-023): verificar retorno a `welcome` sin restaurar progreso en `e2e/match-reload-reset.spec.ts` y `src/app/app.spec.ts` — ambos creados/validados (e2e: 2/2 passing; unit: describe block de `app.spec.ts` para FR-029)
- [X] T086 Implementar manejo explícito de recarga/cierre para iniciar partida nueva en `src/app/app.ts` y `src/app/shared/foundational/state/match-store.service.ts`; registrar listener nativo `beforeunload` cuando `answeredCount() >= 1` para mostrar advertencia del navegador (FR-029), removiéndolo al navegar a `results` o `welcome` (depende de T085) — validado vía unit tests y e2e real (`e2e/match-reload-reset.spec.ts`)
- [X] T087 [P] Medir y registrar evidencia de SC-001 (partida completa <5 min): ejecutar al menos 10 corridas E2E controladas del flujo alias→tema→tablero→6 respuestas→resultados, documentar tiempo por corrida y porcentaje de cumplimiento en `specs/001-trivia-galatea-app/research.md` — evidencia en [research.md §9.3](./research.md) (11.7-13.5s por corrida vía `e2e/full-flow.spec.ts`, muy por debajo de 300s)
- [X] T088 [P] Verificación auditable de A-010/FR-018/FR-019: interceptar todas las peticiones salientes a Gemini en tests e2e y unitarios (usando spy/mock del HttpClient o del SDK de VertexAI) y afirmar que ningún payload contiene el alias del jugador ni campos de sesión distintos del string del tema o del contexto Galatea provisto; documentar evidencia en `specs/001-trivia-galatea-app/research.md` (depende de T027, T031) — test explícito en `question.service.spec.ts` + evidencia en [research.md §9.8](./research.md)
- [~] T089 [DOC] [OPS] Nota operativa de cierre de evento: al finalizar la ventana del evento (~1 mes), rotar o deshabilitar la API key en Google Cloud Console en menos de 24h post-evento, y verificar que la restricción de HTTP Referer haya estado activa durante todo el período (FR-027, A-011). Evidencia: captura de pantalla de la consola con la key deshabilitada archivada en `specs/001-trivia-galatea-app/research.md`. — **protocolo documentado**, ver [research.md §9.5](./research.md), pero **NO ejecutado todavía**: el evento aún no ha ocurrido. ACCIÓN MANUAL PENDIENTE después del evento: responsable operativo debe rotar/deshabilitar la API key en Google Cloud Console dentro de las 24h posteriores al cierre del evento y archivar la captura de pantalla como evidencia
- [X] T090 [US1] [FR-032] **Ajuste operativo 2026-08-25 (modo kiosco)**: retirar el contador local por sesión (`sessionStorage`) y el bloqueo de solicitudes en cliente de `src/app/infrastructure/gemini/gemini-client.service.ts`; mantener resiliencia por timeout/reintentos (FR-024) y delegar control de abuso/cuota a Google Cloud (HTTP Referer restriction, App Check y cuotas/rate limits). Actualizar tests unitarios afectados (`gemini-client.service.spec.ts`, `question.service.spec.ts`, `welcome.page.spec.ts`) para eliminar dependencias del error de límite local.
- [X] T091 [P] Implementar `AriaEscapePipe` (escapa `"` `'` `<` `>` como entidades HTML para atributos `aria-*`) en `src/app/shared/pipes/aria-escape.pipe.ts` + test en `aria-escape.pipe.spec.ts`; aplicar en `tg-question-card` y `tg-question-modal` donde se interpola el alias (FR-030) (depende de T047, T048) — **alcance real verificado (2026-08-21) distinto al descrito**: el alias del jugador NUNCA se interpola en ningún `aria-*` de la app (solo aparece como texto plano en `results.page.html`, que Angular ya escapa por defecto); `tg-question-modal` tampoco interpola texto libre de usuario en su `aria-label` (usa `question().text`, contenido de la pregunta, no input del jugador). El riesgo real y confirmado es `chosenTopic` (tema libre elegido por el jugador, sin restricción de caracteres, solo `maxLength`) interpolado sin escapar en el `aria-label` de `tg-question-card` vía `categoryLabel()`. `AriaEscapePipe` implementado y aplicado únicamente ahí; FR-030 reformulado en spec.md (2026-08-21) para hablar de "texto libre ingresado por el jugador (categoría/tema elegido)" en vez de "alias del jugador", alineando la spec con la implementación real (ver spec.md §Clarifications, sesión "corrección FR-030 post-implementación")
...
- [X] T094 [FR-025] Implementar manejo de pérdida de conectividad diferenciado por estado de partida: registrar listeners `online`/`offline` (en `CurrentMatchStore` + integración en `welcome.page.ts`/`board.page.ts`, sin modificar los archivos protegidos `match-store.port.ts`/`match-store.service.ts`). Criterios de aceptación: (1) si la conectividad falla ANTES de que el tablero esté activo (partida aún no construida/cargada), la app se queda en `welcome` con mensaje amigable y el alias conservado (sin navegar a `board`); (2) si falla DESPUÉS de que el tablero ya está activo (12 preguntas cargadas), la partida NO se reinicia ni pierde progreso — el jugador sigue jugando localmente sobre las cartas ya cargadas, con un aviso visible (no bloqueante) de que la generación por IA no está disponible sin conexión; (3) al recuperar conexión, el aviso desaparece. Incluir tests unitarios para ambos escenarios (per convergence finding E1)
- [X] T095 [FR-005A] Implementar validación runtime de longitud de preguntas generadas por Gemini: enunciado entre 30-180 caracteres, cada opción entre 10-100 caracteres, en `GeminiQuestionMapper`/`isValidRawQuestion` (`gemini-question.mapper.ts`). Las preguntas de Gemini fuera de rango se rechazan (mismo tratamiento que una violación de esquema existente: se descartan y cuentan como fallo de generación, reutilizando el flujo ya probado de `InsufficientGeneratedQuestionsError`/fallback). Actualizar `contracts/gemini-prompt-contract.md` (§2, §3, §4) documentando el rango como parte del esquema esperado y de las instrucciones del prompt. Incluir tests unitarios cubriendo enunciado/opción por debajo y por encima del rango (per convergence finding E2). **Alcance acotado (2026-08-25, finding F1 de /speckit.analyze)**: FR-005A aplica únicamente a preguntas generadas por IA; el banco curado de Galatea (`galatea-question.mapper.ts`, `galatea-questions.json`, `galatea-question-bank.schema.json`) queda explícitamente fuera de esta validación por ser contenido humano revisado — no se modifica ni el mapper ni el schema del banco (ver spec.md §Clarifications, sesión "acotación de alcance FR-005A post-convergencia")
- [X] T096 [US1] [FR-033] Implementar control de sesgo de `correctOptionIndex` para preguntas generadas por IA (tema elegido y fallback Galatea) en `src/app/infrastructure/question/question.service.ts` y/o `src/app/infrastructure/helpers/maps/gemini-question.mapper.ts`: en cada lote de 6 preguntas IA no permitir concentración dominante en un único índice (`>3` en la misma posición 0..3); si ocurre, reordenar opciones preservando la respuesta correcta o regenerar lote antes de iniciar partida.
- [X] T097 [US1] [FR-034] Corregir la política de temas sensibles en `src/app/infrastructure/gemini/topic-safety-policy.ts` aplicando normalización robusta (case-insensitive, sin tildes, variantes comunes/leetspeak básico, tokenización para evitar falsos positivos por subcadenas). Criterios mínimos: bloquear `sexo`, `kamasutra`, `nopor`, `porno`; permitir `computación`.
- [X] T098 [P] [FR-033] Añadir regresiones unitarias para distribución de índices correctos en preguntas IA (sin afectar banco manual) en `src/app/infrastructure/question/question.service.spec.ts` y/o `src/app/infrastructure/helpers/maps/gemini-question.mapper.spec.ts`.
- [X] T099 [P] [FR-034] Añadir regresiones unitarias para moderación de temas sensibles (bloqueos explícitos + casos neutros permitidos + falsos positivos) en `src/app/infrastructure/gemini/topic-safety-policy.spec.ts` (crear si no existe).

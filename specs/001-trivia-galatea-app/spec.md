...
- **FR-005A**: El texto del enunciado de cada pregunta **generada por IA** (Gemini, tanto para el tema elegido como para el fallback de Galatea) DEBE tener entre 30 y 180 caracteres, y cada opción DEBE tener entre 10 y 100 caracteres, para garantizar legibilidad y consistencia visual en la tarjeta. Esta restricción no aplica a las preguntas del banco curado de Galatea (`galatea-questions.json`), cuyo contenido es redactado y revisado manualmente antes del evento.
...
- Q: Al implementar T091 (Phase 7) se verificó en todo `src/` dónde se interpola realmente texto libre del jugador en atributos `aria-*`; ¿el alias es efectivamente ese campo? → A: No. El alias del jugador nunca se interpola en ningún `aria-*` de la app (solo aparece como texto plano en `results.page.html`, ya escapado por Angular por defecto). El campo que sí se interpola sin escapar en un `aria-*` es `chosenTopic` (el tema libre elegido por el jugador, validado solo por longitud vía `Validators.maxLength(60)`, sin restricción de caracteres), a través de `categoryLabel()` en el `aria-label` de `tg-question-card`. FR-030 se corrige para reflejar el campo protegido real; la implementación de `AriaEscapePipe` (T091) se aplicó únicamente ahí.
